<?php
include("../scripts/settings.php");
include("helpers/value_mapper.php");
include("helpers/filter_builder.php");
include("helpers/ncd_status_helper.php");

header('Content-Type: application/json');

$request = $_REQUEST;

// columns
$columns = [];
$resCols = execute_query("SHOW COLUMNS FROM ncd_cooperative_registrations");

while ($c = mysqli_fetch_assoc($resCols)) {
    if (in_array($c['Field'], ['created_at', 'updated_at'])) continue;
    $columns[] = $c['Field'];
}

$where = buildCooperativeFilters($request);

if (!empty($request['search']['value'])) {

    $search = addslashes(trim($request['search']['value']));
    $searchParts = [];

    foreach ($columns as $col) {
        $searchParts[] = "c.$col LIKE '%$search%'";
    }

    $where .= " AND (" . implode(" OR ", $searchParts) . ")";
}

$total = mysqli_fetch_assoc(execute_query("SELECT COUNT(*) t FROM ncd_cooperative_registrations c"))['t'];
$totalFiltered = mysqli_fetch_assoc(execute_query("SELECT COUNT(*) t FROM ncd_cooperative_registrations c $where"))['t'];

$orderCol = $columns[intval($request['order'][0]['column'] ?? 0)] ?? 'id';
$orderDir = $request['order'][0]['dir'] ?? 'desc';

$start = intval($request['start'] ?? 0);
$length = intval($request['length'] ?? 25);

$res = execute_query("
    SELECT c.*
    FROM ncd_cooperative_registrations c
    $where
    ORDER BY c.$orderCol $orderDir
    LIMIT $start, $length
");

$data = [];

$sr = $start + 1;

while ($row = mysqli_fetch_assoc($res)) {

    foreach ($row as $col => $val) {
        $row[$col] = mapDisplayValue($col, $val);
    }


    //Validation Status work
    $statusObj = getValidationStatus($row['id']);

    $row['status_text'] = $statusObj['text'];
    $row['status_color'] = $statusObj['color'];

    $row['id'] = $row['id'];

    $row['sr_no'] = $sr++;

    $data[] = $row;
}

echo json_encode([
    "draw" => intval($request['draw'] ?? 0),
    "recordsTotal" => $total,
    "recordsFiltered" => $totalFiltered,
    "data" => $data
]);

