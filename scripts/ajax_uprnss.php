...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    saveApexWorkProfession($_POST['survey_id']);
...
function saveApexWorkProfession($survey_id)
{
    if (!isset($_POST['other_business_id']))
        return;

    $apex_code = intval($_POST['apex_code']);

    execute_query('DELETE FROM apex_work_profession_info WHERE survey_id="' . $survey_id . '"');

    $totalRows = intval($_POST['other_business_id']);
    ...
    for ($i = 1; $i <= $totalRows; $i++) {
        ...
        $sql = 'INSERT INTO apex_work_profession_info
        (
            survey_id,
            apex_code,
            business_year,
