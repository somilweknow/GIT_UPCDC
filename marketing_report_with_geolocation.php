...
<style>
  ...
  .report-table.concise th, .report-table.concise td {
    padding: 6px 8px;
    font-size: 12px;
    max-width: 150px;
    overflow: hidden;
  ...
  .report-table.concise td:hover { white-space: normal; background: #fdfdfd; }

  .download-buttons { margin-bottom: 12px; }
  ...
  .download-buttons button:hover { opacity: 0.9; }
  ...
  .report-table {
    width: 100%;
    table-layout: auto;   /* allows columns to size by content */
}

.report-table td,
.report-table th {
    white-space: nowrap;  /* keep other columns compact */
}

.report-table td:nth-child(5),
.report-table th:nth-child(5) {
    white-space: normal;  /* allow wrapping only in Samiti Name */
    width: auto;
}
...
<?php
$sql = "SELECT m.*, md.district_name, dv.division_name 
        FROM marketing m 
        LEFT JOIN master_district md ON m.district_id = md.sno 
        LEFT JOIN master_division dv ON m.division_id = dv.sno 
...
if (!empty($_SESSION['division_id'])) {
  ...
  if ($div_ids) $sql .= " AND m.division_id IN (" . implode(',', $div_ids) . ")";
}
if (!empty($_SESSION['district_id'])) {
  $dis_ids = array_map('intval', (array) $_SESSION['district_id']);
  if ($dis_ids) $sql .= " AND m.district_id IN (" . implode(',', $dis_ids) . ")";
}
