...
<style>
    ...
    /* Keep all columns compact */
.report-table th,
.report-table td {
    white-space: nowrap;
}

/* Allow ONLY Samiti Name column to expand & wrap */
            ...
            <table id="general_stat_table" class="report-table">
                <thead>
                    <tr>
                        ...
                        <th style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:300px;">Latitude</th>
                        <th style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:300px;">Longitude</th>
                    </tr>
                </thead>
                ...
                <tbody>
                    ...
                    $sql = "SELECT s.*, dt.district_name, dv.division_name FROM block_union s LEFT JOIN master_district dt ON s.janpad_name = dt.sno LEFT JOIN master_division dv ON s.mandal_name = dv.sno WHERE s.latitude != '' AND 1=1";
                    ...
                    if ($result) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td style='white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100px;'>" . h($row['latitude'] ?? '') . "</td>";
                            echo "<td style='white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:100px;'>" . h($row['longitude'] ?? '') . "</td>";
                            echo "</tr>";
                        }
