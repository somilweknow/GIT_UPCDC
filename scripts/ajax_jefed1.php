		...
		switch($_POST['current_step_count']){
			...
			case 2:{
				...
				foreach ($sanctioned_posts as $post_id => $sanctioned_post) {
					...
					$sql_insert_human = "INSERT INTO apex_si_6_3 (survey_id, post_id, sanctioned_post, vacant_post,working_name, working_period, contract_no, contract_name,created_by, created_at) VALUES ('" . mysqli_real_escape_string($db, $_POST['survey_id']) . "','" . mysqli_real_escape_string($db, $post_id) . "','{$sanctioned_post}', '{$vacant_post}','{$working_name}', '{$working_period}','{$contract_no}', '{$contract_name}','" . mysqli_real_escape_string($db, $_SESSION['user_id']) . "','" . date('Y-m-d H:i:s') . "')";
