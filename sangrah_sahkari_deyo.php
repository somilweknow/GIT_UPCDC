...
if (isset($_GET['ajax_preview'])) {
    ...
    if (!empty($date_from)) {
        ...
        $res_p = execute_query("
            ...
            WHERE p.entry_date BETWEEN '$date_from' AND '$date_to' AND p.district_id IN ($in)");
        ...
        $res_s = execute_query("
            ...
            WHERE s.entry_date BETWEEN '$date_from' AND '$date_to' AND s.district_id IN ($in)");
        ...
        $res_v = execute_query("
            ...
            WHERE v.entry_date BETWEEN '$date_from' AND '$date_to' AND v.district_id IN ($in)");
...
<script>
    ...
    function loadGridData() {
        ...
        document.getElementById('grid_title').textContent =
            fixedDistrictName + ' — दिनांक: ' + f[2]+'-'+f[1]+'-'+f[0] + ' से ' + t[2]+'-'+t[1]+'-'+t[0] + ' (तीनों मॉड्यूल का संयुक्त योग)';
