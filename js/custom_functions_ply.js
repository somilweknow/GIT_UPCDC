...
function calculate_sqmt(value) {
	...
	if(result_unit=='feet'){
		if(unit=='Feet' || unit==''){
			...
			sqmt = height*width;
			price = Math.round((price/10.764)*100)/100;
		}
		unit = 'Feet';
	}
	else if(result_unit=='sqmt'){
		if(unit=='Feet' || unit==''){
			width = width/3.28084;
			height = height/3.28084;
			sqmt = width*height;
