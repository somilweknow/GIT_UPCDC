...
{
	if (empty($_POST['survey_id']) || $_POST['survey_id'] == '') {
		return;
	...
	$basePath = "D:/htdocs/Upcdc/user_data/";
...
function saveApexFinancialInfo($post)
{
	...
	$conn->query("DELETE FROM apex_financial_info WHERE survey_id = $survey_id");
	...
	while (isset($post['sec_3_profit_loss_' . $index])) {
		...
		$stmt->bind_param(
			...
			$survey_id,
			$yearLabel,
			$post['sec_3_profit_loss_' . $index],
			$post['sec_3_gross_amount_' . $index],
			$post['sec_3_net_amount_' . $index],
			$post['sec_3_accumulated_' . $index],
			$post['sec_3_acc_gross_amount_' . $index],
			$post['sec_3_acc_net_amount_' . $index]
		);
...
//    print_r($post); exit;
	...
	$conn->query("DELETE FROM apex_empty_land_info WHERE survey_id = $survey_id");
	...
	for ($i = 1; $i <= $totalRows; $i++) {
		...
		$existingImage = $post['sec_3_c_existing_image_' . $i] ?? '';
		...
		if (isset($files['sec_3_c_image_' . $i]) && $files['sec_3_c_image_' . $i]['error'] == 0) {
			...
			if (in_array($ext, $allowedTypes)) {
				...
				if ($image) {
					...
					$imageName = $newName; // overwrite only if new uploaded
				}
			}
		}
		...
		$stmt = $conn->prepare("
            INSERT INTO apex_empty_land_info
            (
                survey_id,
                sec_3_c_district,
                sec_3_c_area,
