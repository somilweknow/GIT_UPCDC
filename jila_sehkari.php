...
<div class="card" style="margin-top: 40px;">
  ...
  <div class="table-wrap">
    <table class="report-table">
      ...
      <tbody>
      <?php
      $sql = "SELECT j.*, dt.district_name FROM jila_sehkari j LEFT JOIN master_district dt ON j.janpad_name = dt.sno WHERE (j.is_deleted IS NULL OR j.is_deleted = 0)";
        ...
        <tr>
          <td style="display:flex;gap:8px;align-items:center;">
            <a href="?edit=<?= (int)$row['sno'] ?>"
              style="background:#1565c0;color:#fff;padding:6px 5px;border-radius:5px;text-decoration:none;font-weight:700;font-size:13px;">
              ✏️ Edit
            </a>

            <?php if (empty($row['is_deleted']) || $row['is_deleted'] == 0) { ?>
              <a href="?delete=<?= (int)$row['sno'] ?>"
                onclick="return confirm('Are You Sure ?');"
                style="background:#b00020;color:#fff;padding:6px 5px;border-radius:5px;text-decoration:none;font-weight:700;font-size:13px;">
                🗑️ Delete
              </a>
            <?php } else { ?>
              <span style="background:#ccc;color:#555;padding:6px 5px;border-radius:5px;font-weight:700;font-size:13px;">(Deleted)</span>
            <?php } ?>
          </td>
          <td><?= $i++ ?></td>
          <td><?= h($row['ncd_id']) ?></td>
      ...
      <div>
        ...
        <select name="society_status" class="form-select" onchange="onSocietyStatusChange(this)">
          ...
          <option value="closed"     <?= ($form['society_status'] === 'closed') ? 'selected' : '' ?>>परिसमापनाधीन</option>
              <option value="not_applicable" <?= ($form['society_status'] === 'not_applicable') ? 'selected' : '' ?>>स्थापित नही है</option>
...
<div id="samiti-details" class="card" style="display:none;">
  ...
  <input type="text" name="sachiv_type" class="form-control" value="<?= h($form['sachiv_type']) ?>" placeholder="सचिव का प्रकार दर्ज करें">
</div>
  ...
  <div class="card">
    ...
    <div class="form-grid">
      ...
      <div><label class="form-label">व्यवसाय ₹ (लाख)</label>
        <input type="text" name="business_status_amt" class="form-control" value="<?= h($form['business_status_amt']) ?>">
      </div>
