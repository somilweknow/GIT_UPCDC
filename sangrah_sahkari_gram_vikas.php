...
if ($is_sadmin && isset($_GET['ajax_district_data'])) {
    ...
    $result   = ['static' => [], 'daily' => []];
    if ($sel_dist > 0) {
        $rs = execute_query("SELECT * FROM sangrah_sahkari_gram_vikas_static WHERE district_id = $sel_dist");
        if (mysqli_num_rows($rs) > 0) $result['static'] = mysqli_fetch_assoc($rs);

        $rd = execute_query("SELECT * FROM sangrah_sahkari_gram_vikas_daily_collection
...
if (isset($_GET['ajax_preview'])) {
    ...
    if (!empty($date_from)) {
        ...
        $res_p = execute_query("SELECT d.district_name,
                s.total_bakaya, s.bakaya_95k, s.recovery_95k, s.collection_fee,
                s.big_defaulters_count, s.big_defaulters_amount,
                s.big_defaulters_recovery_count, s.big_defaulters_recovery_amount,
            ...
            FROM sangrah_sahkari_gram_vikas_daily_collection c
            JOIN master_district d ON d.sno = c.district_id
            LEFT JOIN sangrah_sahkari_gram_vikas_static s ON s.district_id = c.district_id
...
if (isset($_POST['id']) && $_POST['id'] === 'dr_review') {
    ...
    $rev_date     = mysqli_real_escape_string($db, $_POST['entry_date'] ?? '');
    ...
    if (!$rev_district || !$rev_date || !$rev_status) {
        echo json_encode(['ok' => false, 'msg' => 'अपूर्ण डेटा।'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    $r = execute_query("UPDATE sangrah_sahkari_gram_vikas_daily_collection
        SET status = '$rev_status', dr_remark = '$rev_remark',
            reviewed_by = $current_user_sno, reviewed_at = NOW()
        WHERE district_id = $rev_district AND entry_date = '$rev_date'");
    if ($r)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['id'] ?? '') === 'submit_sahkari') {
    ...
    $collection_fee                 = floatval($_POST['collection_fee']                 ?? 0);
    $big_defaulters_count           = intval($_POST['big_defaulters_count']             ?? 0);
    $big_defaulters_amount          = floatval($_POST['big_defaulters_amount']          ?? 0);
    $big_defaulters_recovery_count  = intval($_POST['big_defaulters_recovery_count']    ?? 0);
    $big_defaulters_recovery_amount = floatval($_POST['big_defaulters_recovery_amount'] ?? 0);
    ...
    $r1 = execute_query("INSERT INTO sangrah_sahkari_gram_vikas_static
        (district_id, total_bakaya, bakaya_95k, recovery_95k, collection_fee,
         big_defaulters_count, big_defaulters_amount,
         big_defaulters_recovery_count, big_defaulters_recovery_amount,
         updated_at, updated_by)
        VALUES ($district_id, $total_bakaya, $bakaya_95k, $recovery_95k, $collection_fee,
                $big_defaulters_count, $big_defaulters_amount,
                $big_defaulters_recovery_count, $big_defaulters_recovery_amount,
        ...
        ON DUPLICATE KEY UPDATE
            ...
            big_defaulters_count           = VALUES(big_defaulters_count),
            big_defaulters_amount          = VALUES(big_defaulters_amount),
            big_defaulters_recovery_count  = VALUES(big_defaulters_recovery_count),
            big_defaulters_recovery_amount = VALUES(big_defaulters_recovery_amount),
            updated_at                     = NOW(),
            updated_by                     = $current_user_sno");

    $r2 = execute_query("INSERT INTO sangrah_sahkari_gram_vikas_daily_collection
...
$res_s = execute_query("SELECT * FROM sangrah_sahkari_gram_vikas_static WHERE district_id = $user_district_id");
if (mysqli_num_rows($res_s) > 0) $edit_static = mysqli_fetch_assoc($res_s);

$res_d = execute_query("SELECT * FROM sangrah_sahkari_gram_vikas_daily_collection
...
?>
    <style>
        ...
        .past-thead th:last-child { display:table-cell !important; }
...
?>
                ...
                <div class="card-body">
                    <div class="col-md-12">
                        <div class="text-center mb-3">
                            <h4><u>उ०प्र० सहकारी ग्राम विकास बैंक की बकाया वसूली</u></h4>
                        </div>
                        <?php echo $msg; echo $success; ?>

                        <form action="" method="post" id="sahkari_form" accept-charset="UTF-8">
                            <input type="hidden" name="id"          value="submit_sahkari">
                            ...
                            <div class="step">
                                <marquee style="font-size:16px; color:red;">
                                    नोट: सहकारी ग्राम विकास बैंक बकाया वसूली का विवरण प्रतिदिन सही-सही भरें। धनराशि लाख रुपये में भरें।
                                </marquee><br>
                            ...
                            <div class="step">
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-3 form-group">
                                            ...
                                            <input type="number" step="0.01" name="total_bakaya" id="total_bakaya"
                                                   class="form-control" placeholder="0.00" oninput="calcAll()"
                                                   value="<?php echo v_s('total_bakaya', $edit_static); ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-3 form-group">
                                            ...
                                            <input type="number" step="0.01" name="recovery_95k" id="recovery_95k"
                                                   class="form-control" placeholder="0.00" oninput="calcAll()"
                                                   value="<?php echo v_s('recovery_95k', $edit_static); ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                            ...
                                            <input type="text" id="pct_vasuli" class="form-control calc-field" readonly
                                                   value="<?php
                                                   $bk  = floatval($edit_static['bakaya_95k']   ?? 0);
                                                   $rec = floatval($edit_static['recovery_95k'] ?? 0);
                                                   echo ($bk > 0 ? round($rec*100/$bk,2) : 0) . ' %';
                                                   ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                            <label>प्राप्त संग्रह शुल्क (लाख रु.)</label>
                                            <input type="number" step="0.01" name="collection_fee" id="collection_fee"
                                                   class="form-control" placeholder="0.00" oninput="calcAll()"
                                                   value="<?php echo v_s('collection_fee', $edit_static); ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-3 form-group">
                                            <label>संख्या</label>
                                            <input type="number" min="0" name="big_defaulters_count" id="big_defaulters_count"
                                                   class="form-control" placeholder="0"
                                                   value="<?php echo v_s('big_defaulters_count', $edit_static); ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                            <label>धनराशि (लाख रु.)</label>
                                            <input type="number" step="0.01" name="big_defaulters_amount" id="big_defaulters_amount"
                                                   class="form-control" placeholder="0.00" oninput="calcAll()"
                                                   value="<?php echo v_s('big_defaulters_amount', $edit_static); ?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-3 form-group">
                                            ...
                                            <input type="number" min="0" name="big_defaulters_recovery_count" id="big_defaulters_recovery_count"
                                                   class="form-control" placeholder="0"
                                                   value="<?php echo v_s('big_defaulters_recovery_count', $edit_static); ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                            <label>वसूली धनराशि (लाख रु.)</label>
                                            <input type="number" step="0.01" name="big_defaulters_recovery_amount" id="big_defaulters_recovery_amount"
                                                   class="form-control" placeholder="0.00" oninput="calcAll()"
                                                   value="<?php echo v_s('big_defaulters_recovery_amount', $edit_static); ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                            <label>एक लाख से बड़े बकायेदारों से वसूली प्रतिशत</label>
                                            <input type="text" id="pct_big" class="form-control calc-field" readonly
                                                   value="<?php
                                                   $bda  = floatval($edit_static['big_defaulters_amount']          ?? 0);
                                                   $bdra = floatval($edit_static['big_defaulters_recovery_amount'] ?? 0);
                            ...
                            <div class="step">
                                <h4>3. दैनिक वसूली एवं भुगतान का विवरण (दिनांक: <?php echo date('d-m-Y'); ?>)</h4>
                                <div class="col-sm-12">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-4 form-group">
                                            ...
                                            <input type="number" step="0.01" name="daily_recovery_95k" id="daily_recovery_95k"
                                                   class="form-control" placeholder="0.00" oninput="calcBalance()"
                                    ...
                                    <?php if (!$is_sadmin && !$is_dr): ?>
                                    <div class="info-box mb-2" id="today_status_box" style="display:none;">
                                        आज के डेटा की स्थिति: <span id="today_status_badge"></span>
                                        <span id="today_dr_remark_wrap" style="display:none;"> | DR टिप्पणी: <strong id="today_dr_remark"></strong></span>
                                    </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="row align-items-end mb-2">
                                        ...
                                        <div class="col-sm-4 form-group">
                                            ...
                                            <span id="prog_loading" class="ml-2" style="display:none; color:#FF8E00; font-size:13px;">⏳ लोड हो रहा है...</span>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function sadminDistrictChange(districtId) {
            ...
            xhr.onload = function () {
                ...
                try {
                    ...
                    setField('collection_fee',                 s.collection_fee                 || '');
                    setField('big_defaulters_count',           s.big_defaulters_count           || '');
                    setField('big_defaulters_amount',          s.big_defaulters_amount          || '');
                    setField('big_defaulters_recovery_count',  s.big_defaulters_recovery_count  || '');
                    setField('big_defaulters_recovery_amount', s.big_defaulters_recovery_amount || '');
        ...
        function calcAll() {
            var p5  = pct(gv('bakaya_95k'),                   gv('total_bakaya'));
            var p7  = pct(gv('recovery_95k'),                 gv('bakaya_95k'));
            var p13 = pct(gv('big_defaulters_recovery_amount'), gv('big_defaulters_amount'));
        ...
        function loadPastData() {
            ...
            var loading = document.getElementById('prog_loading');
            ...
            xhr.onload = function () {
                ...
                try {
                    ...
                    var isToday = (dateFrom === pdate_today && dateTo === pdate_today);

                    rows.forEach(function (r) {
                        var tb   = parseFloat(r.total_bakaya  || 0);
                        var bk   = parseFloat(r.bakaya_95k    || 0);
                        var rec  = parseFloat(r.recovery_95k  || 0);
                        var bda  = parseFloat(r.big_defaulters_amount          || 0);
                        var bdra = parseFloat(r.big_defaulters_recovery_amount || 0);
                        ...
                        if (isDr) {
                            ...
                            actionCell =
                                '<td>' +
                                '<textarea id="remark_' + distId + '_' + rDate + '" class="form-control form-control-sm mb-1" rows="1" placeholder="टिप्पणी (वैकल्पिक)" style="min-width:140px;">' + (r.dr_remark || '') + '</textarea>' +
                                '<button class="btn btn-success btn-sm mr-1" onclick="submitReview(' + distId + ',\'' + rDate + '\',\'approved\')">✔ स्वीकृत</button>' +
                                '<button class="btn btn-danger btn-sm" onclick="submitReview(' + distId + ',\'' + rDate + '\',\'rejected\')">✘ अस्वीकृत</button>' +
                                '</td>';
                        }

                        if (isToday && !isSadmin && !isDr) {
                            var sb = document.getElementById('today_status_box');
                            var sbadge = document.getElementById('today_status_badge');
                            var sremark = document.getElementById('today_dr_remark');
                            var sremarkWrap = document.getElementById('today_dr_remark_wrap');
                            if (sb) sb.style.display = 'block';
                            if (sbadge) sbadge.innerHTML = stBadge;
                            if (r.dr_remark && sremark) {
                                sremark.textContent = r.dr_remark;
                                sremarkWrap.style.display = 'inline';
                            }
                        }

                        var ed = (rDate || '').split('-');
                        var displayDate = ed.length === 3 ? ed[2]+'-'+ed[1]+'-'+ed[0] : (rDate || '-');
                        ...
                        tbody.innerHTML +=
                            ...
                            '<td>' + p7  + ' %</td>' +
                            '<td>' + (r.collection_fee || '-') + '</td>' +
                            '<td>' + (r.big_defaulters_count             || '-') + '</td>' +
                            '<td>' + (r.big_defaulters_amount            || '-') + '</td>' +
                            '<td>' + (r.big_defaulters_recovery_count    || '-') + '</td>' +
                            '<td>' + (r.big_defaulters_recovery_amount   || '-') + '</td>' +
                            '<td>' + p13 + ' %</td>' +
        ...
        function submitReview(districtId, entryDate, status) {
            var remark = '';
            var el = document.getElementById('remark_' + districtId + '_' + entryDate);
            ...
            var body = 'id=dr_review'
                ...
                + '&entry_date='  + encodeURIComponent(entryDate)
                + '&status='      + status
