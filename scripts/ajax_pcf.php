...
elseif($id=='submit_form_pcf'){
	...
	if($_POST['survey_id']==''){
        $sql = 'INSERT INTO `apex_si_1_1` (`apex_id`,`longitude`,`latitude`,`committee_status`,`email_id`,`photo_id`,`society_registration_no`,`society_registration_date`,`pan_no`, `tan_no`, `gst_no`, `mobile_no`, `website`) VALUES ("' . $_POST['apex_code'] . '","'.$_POST['longitude'].'","'.$_POST['latitude'].'","'.$_POST['committee_status'].'","'.$_POST['email_id'].'","'.$_POST['photo_id'].'","'.$_POST['society_registration_no'].'","'.$_POST['society_registration_date'].'", "'.$_POST['pan_no'].'","'.$_POST['tan_no'].'", "'.$_POST['gst_no'].'","'.$_POST['mobile_no'].'", "'.$_POST['website'].'")';
		// echo $_POST['apex_id'];
		execute_query($sql);
		if(mysqli_error($db)){
		...
		switch($_POST['current_step_count']){
			...
			case 1:{
                ...
                for ($i = 1; $i <= $_POST['regional_office_count']; $i++) {
                    $office_name = isset($_POST['office_name_' . $i]) ? $_POST['office_name_' . $i] : '';
                    $district = isset($_POST['district_' . $i]) ? $_POST['district_' . $i] : '';
                    $division = isset($_POST['division_' . $i]) ? $_POST['division_' . $i] : '';
                    $tehsil = isset($_POST['tehsil_' . $i]) ? $_POST['tehsil_' . $i] : '';
                    $address = isset($_POST['address_' . $i]) ? $_POST['address_' . $i] : '';
                    $phone = isset($_POST['phone_' . $i]) ? $_POST['phone_' . $i] : '';
                    $pincode = isset($_POST['pincode_' . $i]) ? $_POST['pincode_' . $i] : '';
				...
				for ($i = 1; $i <= $_POST['purchase_sale_row_count']; $i++) {
					...
					$sql = 'INSERT INTO apex_si_7_3
							(wheat_purchase, rice_purchase, seed, fertilizer, godown_rent, nefed, 
							farmer_service_center, other_business, survey_id)
							VALUES
							("' . $wheat . '","' . $rice . '","' . $seed . '","' . $fertilizer . '",
							"' . $rent . '","' . $nefed . '","' . $fsc . '","' . $other . '",
							"' . $_POST['survey_id'] . '")';
				...
				for ($i = 1; $i <= $_POST['sec_7_row_count']; $i++) {
					$business_name = $_POST['sec_7_business_name_' . $i] ?? '';
					$annual_target = $_POST['sec_7_annual_target_' . $i] ?? '';
					$achievement   = $_POST['sec_7_achievement_' . $i] ?? '';

					echo $sql = 'INSERT INTO apex_si_7_4 (survey_id, business_name, annual_target, achievement) 
							VALUES ("' . $_POST['survey_id'] . '","' . e($db, $business_name) . '","' . e($db, $annual_target) . '","' . e($db, $achievement) . '")';
					execute_query($sql);
				}

				if (mysqli_error($db)) {
					$data[] = ["id" => "error", "error" => "Unable to save yearly business details."];
				} else {
					$data[] = ["id" => "update", "msg" => "Yearly business details saved successfully."];
				}
