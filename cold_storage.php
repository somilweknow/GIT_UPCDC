...
// Data saving now handled via AJAX in ajax_cold_storage.php
...
// Pre-fetch IDs for edit mode to populate dropdowns
$edit_division_id = '';
$edit_district_id = '';
$edit_tehseel_id = '';
$edit_block_id = '';
...
if ($is_edit) {
    if (!empty($edit_data['district_name'])) {
        $dist_res = execute_query("SELECT sno, division_id FROM master_district WHERE district_name = '" . mysqli_real_escape_string($db, trim($edit_data['district_name'] ?? '')) . "' OR sno = '" . mysqli_real_escape_string($db, trim($edit_data['district_name'] ?? '')) . "'");
    ...
    if (!empty($edit_data['tehseel_name'])) {
        $teh_res = execute_query("SELECT sno, district_id FROM master_tehseel WHERE tehseel_name = '" . mysqli_real_escape_string($db, trim($edit_data['tehseel_name'] ?? '')) . "' OR sno = '" . mysqli_real_escape_string($db, trim($edit_data['tehseel_name'] ?? '')) . "'");
        if ($teh_row = mysqli_fetch_assoc($teh_res)) {
            $edit_tehseel_id = $teh_row['sno'];
            if (!$edit_district_id) {
                $edit_district_id = $teh_row['district_id'];
                // Since we found district ID from tehseel, also get division ID from district
                $dist_row_fallback = mysqli_fetch_assoc(execute_query("SELECT division_id FROM master_district WHERE sno = '$edit_district_id'"));
                if ($dist_row_fallback) $edit_division_id = $dist_row_fallback['division_id'];
            }
        }
    }

    if (!empty($edit_data['block_name'])) {
        $blk_res = execute_query("SELECT sno, tehseel_id FROM master_block WHERE block_name = '" . mysqli_real_escape_string($db, trim($edit_data['block_name'] ?? '')) . "' OR sno = '" . mysqli_real_escape_string($db, trim($edit_data['block_name'] ?? '')) . "'");
        if ($blk_row = mysqli_fetch_assoc($blk_res)) {
            $edit_block_id = $blk_row['sno'];
            if (!$edit_tehseel_id) $edit_tehseel_id = $blk_row['tehseel_id'];
            if (!$edit_district_id) {
                $teh_row_fallback = mysqli_fetch_assoc(execute_query("SELECT district_id FROM master_tehseel WHERE sno = '".$blk_row['tehseel_id']."'"));
                if ($teh_row_fallback) {
                    $edit_district_id = $teh_row_fallback['district_id'];
                    $dist_row_fallback2 = mysqli_fetch_assoc(execute_query("SELECT division_id FROM master_district WHERE sno = '$edit_district_id'"));
                    if ($dist_row_fallback2) $edit_division_id = $dist_row_fallback2['division_id'];
                }
            }
        }
    }
    ...
    if (!empty($edit_data['society_name'])) {
        $soc_res = execute_query("SELECT sno, col2 as dist_id FROM test2 WHERE col4 = '" . mysqli_real_escape_string($db, trim($edit_data['society_name'] ?? '')) . "' OR sno = '" . mysqli_real_escape_string($db, trim($edit_data['society_name'] ?? '')) . "'");
...
if (isset($_GET['delete_id']) && !empty($_GET['delete_id'])) {
    if (isset($_SESSION['username']) && ($_SESSION['user_type'] == 'ar' || $_SESSION['usertype'] == '3' || $_SESSION['usertype'] == 'sadmin')) {
        ...
        execute_query("DELETE FROM cold_storage_entries WHERE id = '$delete_id'");
        header("Location: cold_storage.php");
        exit;
    }
}
...
// We use references to $edit_data for form rows to simplify logic and maintain consistency
$row_invoice = &$edit_data;
$row_3_1 = &$edit_data;
$row_new_plot = &$edit_data;
$sides_data = [];
$row_2_1 = &$edit_data;
$row_3_5_1 = &$edit_data;
$row_new_scheme = &$edit_data;

$storage_cap_json = (isset($edit_data['storage_capacity_data']) && !empty($edit_data['storage_capacity_data'])) ? json_decode($edit_data['storage_capacity_data'], true) : [];
if (empty($storage_cap_json)) $storage_cap_json = ['count' => 1];
$row_3_4 = &$storage_cap_json;

$row_7_6 = ['count' => 1];

$vacant_land_json = (isset($edit_data['vacant_land_data']) && !empty($edit_data['vacant_land_data'])) ? json_decode($edit_data['vacant_land_data'], true) : [];
if (empty($vacant_land_json)) $vacant_land_json = ['sec_3_c_id' => 1];
$row_3_5 = &$vacant_land_json;
...
<div class="row">
    <div class="col-md-12">
                            ...
                            <div class="form-group mb-2">
                                ...
                                <select name="f_district" id="f_district" class="form-control"
                                    ...
                                    if (isset($_GET['f_division']) && $_GET['f_division'] != '') {
                                        ...
                                        if (isset($_SESSION['user_type']) && $_SESSION['user_type'] == 'ar' && isset($_SESSION['district_id']) && is_array($_SESSION['district_id'])) {
                                            $dist_sql .= " AND sno IN (" . implode(",", $_SESSION['district_id']) . ")";
                                        }
                        ...
                        <tbody>
                            ...
                            if (mysqli_num_rows($check_table) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($entries)) {
                                    ...
                                    if (isset($_SESSION['username']) && ($_SESSION['user_type'] == 'ar' || $_SESSION['usertype'] == '3' || $_SESSION['usertype'] == 'sadmin')) {
                                        echo "  <a href='?edit_id={$row['id']}' class='btn' style='background:#007bff; color:#fff; padding:4px 8px; border-radius:4px; font-size:11px;' title='Edit'><i class='fa fa-edit'></i> Edit</a>
        ...
        <div id="successCard" class="card" style="display:none;">
            <div class="card-header">
                <!-- <h4 class="card-title text-center" style="color:#10b981;">&#10003; सफलता (Success)</h4> -->
            </div>
            <div class="card-body">
                <div id="success">
                    <div class="text-center">
                        <h4>प्रपत्र सफलतापूर्वक भरा गया</h4>
                        <p>आपने प्रपत्र सफलतापूर्वक भर लिया है। अब प्रपत्र को उच्चाधिकारी के पास सत्यापन हेतु भेजा जाना है। कृपया सत्यापन पर भेजने से पहले नीचे दिए गए बटन से फार्म खोलकर दोबारा जांच लें। सब कुछ सही होने की स्थिति में नीचे दिए गए बटन के माध्यम से सत्यापन के लिए आगे भेजें।</p>
                        <button id="preview_btn" class="btn btn-info" onclick="window.open('cold_storage.php', '_blank');">प्रपत्र दोबारा जांचने के लिए देखें</button>
                    </div>
            ...
            <div id="formCard" class="card" style="display:none;">
                <div class="card-body">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-md-9">
                                <h4 class="card-title" style="font-weight: bold; margin: 0;">
                                    शीतगृह विवरण (Cold Storage Details)
                                </h4>
                            </div>
                            <div class="col-md-3 text-right">
                                <button type="button" id="backToHistoryBtn" class="btn" 
                                    style="background-color: #6c757d; border-color: #6c757d; color: white; background-image: none !important; font-size: 14px; padding: 8px 10px;">
                                    <i class="fa fa-arrow-left"></i> वापस जाएं (Back)
                                </button>
                            </div>
                        </div>
                    </div>
                    <form id="coldStorageForm" enctype="multipart/form-data">
                        <?php if ($is_edit): ?>
                            <input type="hidden" name="edit_id" value="<?php echo $edit_id; ?>">
                            <input type="hidden" name="old_district_name" value="<?php echo ($edit_data['district_name'] ?? ''); ?>">
                            <input type="hidden" name="old_tehseel_name" value="<?php echo ($edit_data['tehseel_name'] ?? ''); ?>">
                            <input type="hidden" name="old_block_name" value="<?php echo ($edit_data['block_name'] ?? ''); ?>">
                            <input type="hidden" name="old_society_name" value="<?php echo ($edit_data['society_name'] ?? ''); ?>">
                        <?php endif; ?>
                        <input type="hidden" name="old_building_photo" value="<?php echo htmlspecialchars(($edit_data['building_photo'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="old_ownership_photo" value="<?php echo htmlspecialchars(($edit_data['ownership_photo'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="old_sec_3_approach_image" value="<?php echo htmlspecialchars(($edit_data['sec_3_approach_image'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="mb-2">
                                        <input type="text" name="lat" id="lat" class="form-control" placeholder="Latitude"
                                            value="<?php echo ($edit_data['latitude'] ?? ''); ?>">
                                    </div>
                                    <div class="mb-2">
                                        <input type="text" name="long" id="long" class="form-control" placeholder="Longitude"
                                            value="<?php echo ($edit_data['longitude'] ?? ''); ?>">
                                    </div>
                                    <input type="hidden" name="latitude" id="latitude" value="<?php echo ($edit_data['latitude'] ?? ''); ?>">
                                    <input type="hidden" name="longitude" id="longitude" value="<?php echo ($edit_data['longitude'] ?? ''); ?>">
                                    <div>
                            ...
                            <div class="col-md-9">
                                <div class="form-group">
                                    ...
                                    <iframe id="googlemap"
                                        src="https://maps.google.com/maps?q=<?php echo ($edit_data['latitude'] ?? '') . ',' . ($edit_data['longitude'] ?? ''); ?>&hl=en&z=13&amp;output=embed"
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="division_name" id="division_name" class="form-control" required
                                        ...
                                        while ($row_div = mysqli_fetch_assoc($res_div)) {
                                            $selected = ($edit_division_id == $row_div['sno']) ? 'selected' : '';
                                            echo '<option value="' . ($row_div['sno'] ?? '') . '" ' . $selected . '>' . ($row_div['division_name'] ?? '') . '</option>';
                                        }
                                        ?>
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="district_name" id="district_name" class="form-control" required
                                        ...
                                        <?php
                                        if ($edit_division_id) {
                                            $dist_sql = "SELECT * FROM master_district WHERE division_id = '$edit_division_id'";
                                            if (isset($_SESSION['user_type']) && $_SESSION['user_type'] == 'ar' && isset($_SESSION['district_id']) && is_array($_SESSION['district_id'])) {
                                                $dist_sql .= " AND sno IN (" . implode(",", $_SESSION['district_id']) . ")";
                                            }
                                            $dist_res = execute_query($dist_sql);
                                            while ($d_row = mysqli_fetch_assoc($dist_res)) {
                                                $selected = ($edit_district_id == $d_row['sno']) ? 'selected' : '';
                                                echo '<option value="' . $d_row['sno'] . '" ' . $selected . '>' . $d_row['district_name'] . '</option>';
                                            }
                                        }
                                        ?>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="tehseel_name" id="tehseel_name" class="form-control"
                                        ...
                                        <?php
                                        if ($edit_district_id) {
                                            $teh_res = execute_query("SELECT * FROM master_tehseel WHERE district_id = '$edit_district_id'");
                                            while ($t_row = mysqli_fetch_assoc($teh_res)) {
                                                $selected = ($edit_tehseel_id == $t_row['sno']) ? 'selected' : '';
                                                echo '<option value="' . $t_row['sno'] . '" ' . $selected . '>' . $t_row['tehseel_name'] . '</option>';
                                            }
                                        }
                                        ?>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="block_name" id="block_name" class="form-control"
                                        ...
                                        <?php
                                        if ($edit_tehseel_id) {
                                            $blk_res = execute_query("SELECT * FROM master_block WHERE tehseel_id = '$edit_tehseel_id'");
                                            while ($b_row = mysqli_fetch_assoc($blk_res)) {
                                                $selected = ($edit_block_id == $b_row['sno']) ? 'selected' : '';
                                                echo '<option value="' . $b_row['sno'] . '" ' . $selected . '>' . $b_row['block_name'] . '</option>';
                                            }
                                        }
                                        ?>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="society_name" id="society_name" class="form-control">
                                        ...
                                        <?php
                                        if ($edit_block_id) {
                                            $soc_res = execute_query("SELECT * FROM test2 WHERE col6 = '$edit_block_id' AND col3='1'");
                                            while ($s_row = mysqli_fetch_assoc($soc_res)) {
                                                $selected = ($edit_society_id == $s_row['sno']) ? 'selected' : '';
                                                echo '<option value="' . $s_row['sno'] . '" ' . $selected . '>' . $s_row['col4'] . '</option>';
                                            }
                                        }
                                        ?>
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="capacity" class="form-control"
                                        value="<?php echo ($edit_data['capacity'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <?php
                                $b_photo = ($edit_data['building_photo'] ?? '');
                        ...
                        <div class="col-md-12" id="inactivity_div"
                            style="<?php echo (($edit_data['is_active'] ?? '') == 'inactive') ? 'display:block;' : 'display:none;'; ?>">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="inactivity_reason" class="form-control">
                                            ...
                                            <option value="निष्प्रयोग" <?php if (($edit_data['inactivity_reason'] ?? '') == 'निष्प्रयोग')
                                                echo 'selected'; ?>>निष्प्रयोज्य</option>
                                            <option value="मरम्मत योग्य" <?php if (($edit_data['inactivity_reason'] ?? '') == 'मरम्मत योग्य')
                                                echo 'selected'; ?>>मरम्मत योग्य</option>
                                            <option value="विवादित" <?php if (($edit_data['inactivity_reason'] ?? '') == 'विवादित')
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="inactivity_date" class="form-control"
                                            value="<?php echo ($edit_data['inactivity_date'] ?? ''); ?>" placeholder="वर्ष दर्ज करें">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <textarea name="inactivity_description" class="form-control"
                                            rows="2"><?php echo ($edit_data['inactivity_description'] ?? ''); ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-sm-12">
                            ...
                            <div class="col-sm-12">
                                ...
                                <div class="col-sm-3 form-group">
                                    <!-- <label>शीतगृह के भवन का स्वामित्व </label> -->
                                        <select name="ownership_type" id="ownership_type" tabindex="<?php echo $tab++; ?>"
                                            class="form-control"
                                            onChange="hide_show(this.value, '#rented_details', 'rent'); hide_show(this.value, '#other_details', 'other'); hide_show(this.value, '#sec_3_image', ['own', 'rent', 'other']);">
                                            <option value="">--Select--</option>
                                            <option value="own" <?php $sec_3_rented_display = 'none';
                                            $sec_3_other_display = 'none';
                                            if (($edit_data['ownership_type'] ?? '') == 'own') {
                                                echo ' selected="selected" ';
                                            } ?>> स्वामित्व में है</option>
                                            <option value="rent" <?php if (($edit_data['ownership_type'] ?? '') == 'rent') {
                                                echo ' selected="selected" ';
                                                $sec_3_rented_display = 'flex';
                                            } ?>>किराये पर है
                                            </option>
                                            <option value="other" <?php if (($edit_data['ownership_type'] ?? '') == 'other') {
                                                echo ' selected="selected" ';
                                    ...
                                    <div class="col-sm-3 form-group" id="sec_3_image" style="display:none;">
                                        ...
                                        <input type="file" accept=".jpg, .jpeg, .gif, .png, .bmp"
                                            name="ownership_photo" id="ownership_photo"
                                            tabindex="<?php echo $tab++; ?>" class="form-control">
                                    </div>
                                    <?php
                                    $own_photo = ($edit_data['ownership_photo'] ?? '');
                                    if (!empty($own_photo) && file_exists($own_photo)) {
                                        ?>
                                        <div class="col-sm-3 form-group">
                                            <img src="<?php echo $own_photo; ?>"
                                                class="img-fluid img-thumbnail" style="height:50px;"
                                                id="sec_3_a_image_uploaded">
                                            <label><a href="<?php echo $own_photo; ?>"
                                    ...
                                    ?>
                                    <div id="rented_details" style="display: <?php echo $sec_3_rented_display; ?>">
                                        <div class="col-sm-3 form-group">
                                            <label> भवन का मासिक किराया</label>
                                            <input name="monthly_rent" id="monthly_rent"
                                                tabindex="<?php echo $tab++; ?>" class="form-control"
                                                value="<?php echo ($edit_data['monthly_rent'] ?? ''); ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                            <label>शीतगृह का क्षेत्रफल (स्क्वायर फिट में)</label>
                                            <input name="building_area_sqft" id="building_area_sqft"
                                                tabindex="<?php echo $tab++; ?>" class="form-control chk_decimal"
                                                value="<?php echo ($edit_data['building_area_sqft'] ?? ''); ?>">
                                        </div>

                                    </div>
                                    <div id="other_details" style="display: <?php echo $sec_3_other_display; ?>">
                                        <div class="col-sm-3 form-group">
                                            <label>कृपया विवरण दर्ज करें</label>
                                            <input name="ownership_desc" id="ownership_desc"
                                                tabindex="<?php echo $tab++; ?>" class="form-control"
                                                value="<?php echo ($edit_data['ownership_desc'] ?? ''); ?>">
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-3 form-group">चारदिवारी (बाऊण्डरी वाल)
                                            ...
                                            <select name="is_litigation" id="is_litigation" tabindex="<?php echo $tab++; ?>"
                                                ...
                                                <option value="">--Select--</option>
                                                <option value="yes" <?php echo (($edit_data['is_litigation'] ?? '') == 'yes') ? ' selected="selected"' : ''; ?> style="background:#0f0">हाँ</option>
                                                <option value="no" <?php echo (($edit_data['is_litigation'] ?? '') == 'no') ? ' selected="selected"' : ''; ?> style="background:#f00">नहीं</option>
                                            </select>
                                        </div>

                                        <div class="col-sm-3 form-group" id="court_case_div"
                                            style="display: <?php echo (($edit_data['is_litigation'] ?? '') == 'yes') ? 'block' : 'none'; ?>">
                                            <label>किस न्यायालय में विचाराधीन है?</label>
                                            <select name="court_case" id="court_case" class="form-control"
                                                tabindex="<?php echo $tab++; ?>">
                                                <option value="">-- चुनें --</option>
                                                <option value="Revenue Court" <?php echo (($edit_data['court_case'] ?? '') == 'Revenue Court') ? ' selected="selected"' : ''; ?>>राजस्व न्यायालय (Revenue
                                                    Court)</option>
                                                <option value="Civil Court" <?php echo (($edit_data['court_case'] ?? '') == 'Civil Court') ? ' selected="selected"' : ''; ?>>सिविल न्यायालय (Civil Court)</option>
                                                <option value="High Court" <?php echo (($edit_data['court_case'] ?? '') == 'High Court') ? ' selected="selected"' : ''; ?>>हाई कोर्ट (High Court)</option>
                                                <option value="Supreme Court" <?php echo (($edit_data['court_case'] ?? '') == 'Supreme Court') ? ' selected="selected"' : ''; ?>>उच्चतम न्यायालय (Supreme
                                                    Court)</option>
                                                <option value="Other" <?php echo (($edit_data['court_case'] ?? '') == 'Other') ? ' selected="selected"' : ''; ?>>अन्य </option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-sm-3 form-group" id="litigation_start_year_div"
                                            ...
                                            <select name="litigation_start_year" id="litigation_start_year"
                                                ...
                                                for ($y = 1900; $y <= $currentYear; $y++) {
                                                    $selected = (($edit_data['litigation_start_year'] ?? '') == $y) ? 'selected="selected"' : '';
                                        ...
                                        <div class="col-sm-3 form-group" id="litigation_current_status_div"
                                            ...
                                            <input type="text" name="litigation_current_status"
                                                ...
                                                tabindex="<?php echo $tab++; ?>"
                                                value="<?php echo ($edit_data['litigation_current_status'] ?? ''); ?>">
                                        </div>
                                    </div>
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-3 form-group">
                                            ...
                                            <input type="text" name="sec_new_plot_value" id="sec_new_plot_value"
                                                tabindex="<?php echo $tab++; ?>" class="form-control"
                                                data-type="भूमि का मूल्य"
                                                value="<?php echo ($row_new_plot['sec_new_plot_value'] ?? ''); ?>">
                                        </div>
                                        <div class="col-sm-3 form-group">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-3 form-group" id="sec_3_d_boundry_height_1"
                                                ...
                                                <input type="number" name="sec_3_d_boundry_height"
                                                    ...
                                                    class="form-control" value="<?php echo ($row_3_1['sec_3_d_boundry_height'] ?? ''); ?>">
                                            </div>
                                            <div class="col-sm-3 form-group">
                        ...
                        <div style="margin-bottom: 20px; margin-left: 10px; margin-right: 10px;">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="elec_conn" id="elec_conn" class="form-control"
                                            ...
                                            <option value="">-- चुनें --</option>
                                            <option value="yes" <?php if (($edit_data['elec_conn'] ?? '') == 'yes')
                                                echo 'selected'; ?>>हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['elec_conn'] ?? '') == 'no')
                                                echo 'selected'; ?>>नहीं
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-4 elec_yes_row"
                                    style="<?php echo (($edit_data['elec_conn'] ?? '') == 'yes') ? 'display:block;' : 'display:none;'; ?>">
                                    <div class="form-group">
                                        <label>यदि है तो चालू है या नहीं ?</label>
                                        <select name="elec_working" class="form-control"
                                            onchange="toggleElecWorking(this.value)">
                                            <option value="">-- चुनें --</option>
                                            <option value="yes" <?php if (($edit_data['elec_working'] ?? '') == 'yes')
                                                echo 'selected'; ?>>
                                                हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['elec_working'] ?? '') == 'no')
                                                echo 'selected'; ?>>नहीं
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-4 elec_no_row"
                                    style="<?php echo (($edit_data['elec_conn'] ?? '') == 'no') ? 'display:block;' : 'display:none;'; ?>">
                                    <div class="form-group">
                                        <label>यदि नहीं है तो प्रस्ताव</label>
                                        <textarea name="elec_proposal" class="form-control"
                                            rows="1"><?php echo ($edit_data['elec_proposal'] ?? ''); ?></textarea>
                                    </div>
                                </div>
                                <!-- Show if working NO -->
                                <div class="col-md-4 elec_working_no"
                                    style="<?php echo (($edit_data['elec_working'] ?? '') == 'no') ? 'display:block;' : 'display:none;'; ?>">
                                    <div class="form-group">
                                        <label>यदि चालू नहीं है तो कारण</label>
                                        <input type="text" name="elec_not_working_reason" class="form-control"
                                            value="<?php echo ($edit_data['elec_not_working_reason'] ?? ''); ?>">
                                    </div>
                                </div>
                                <!-- Show if working YES -->
                                <div class="col-md-4 elec_working_yes"
                                    style="<?php echo (($edit_data['elec_working'] ?? '') == 'yes') ? 'display:block;' : 'display:none;'; ?>">
                                    <div class="form-group">
                                        ...
                                        <select name="elec_bill_regular" class="form-control"
                                            ...
                                            <option value="">-- चुनें --</option>
                                            <option value="yes" <?php if (($edit_data['elec_bill_regular'] ?? '') == 'yes')
                                                echo 'selected'; ?>>हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['elec_bill_regular'] ?? '') == 'no')
                                                echo 'selected'; ?>>नहीं
                                            </option>
                                        </select>
                                    </div>
                            ...
                            <div class="row elec_bill_no"
                                style="<?php echo (($edit_data['elec_bill_regular'] ?? '') == 'no' && ($edit_data['elec_working'] ?? '') == 'yes') ? 'display:flex;' : 'display:none;'; ?>">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>कितने माह से भुगतान नहीं है?</label>
                                        <input type="number" name="elec_months_due" class="form-control"
                                            value="<?php echo ($edit_data['elec_months_due'] ?? ''); ?>">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="0.01" name="elec_outstanding" class="form-control"
                                            value="<?php echo ($edit_data['elec_outstanding'] ?? ''); ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="margin-bottom: 20px; margin-left: 10px; margin-right: 10px;">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="solar_conn" class="form-control"
                                            ...
                                            <option value="">-- चुनें --</option>
                                            <option value="yes" <?php if (($edit_data['solar_conn'] ?? '') == 'yes')
                                                echo 'selected'; ?>>हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['solar_conn'] ?? '') == 'no')
                                                echo 'selected'; ?>>नहीं
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-4 solar_yes_row"
                                    style="<?php echo (($edit_data['solar_conn'] ?? '') == 'yes') ? 'display:block;' : 'display:none;'; ?>">
                                    <div class="form-group">
                                        <label>यदि है तो चालू है या नहीं ?</label>
                                        <select name="solar_working" class="form-control"
                                            onchange="toggleSolarWorking(this.value)">
                                            <option value="">-- चुनें --</option>
                                            <option value="yes" <?php if (($edit_data['solar_working'] ?? '') == 'yes')
                                                echo 'selected'; ?>>
                                                हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['solar_working'] ?? '') == 'no')
                                                echo 'selected'; ?>>
                                                नहीं
                                            </option>
                                        </select>
                                ...
                                <div class="col-md-4 solar_working_yes"
                                    style="<?php echo (($edit_data['solar_working'] ?? '') == 'yes') ? 'display:block;' : 'display:none;'; ?>">
                                    <div class="form-group">
                                        <label>बैट्री की स्थिति</label>
                                        <select name="solar_battery_status" class="form-control">
                                            <option value="good" <?php if (($edit_data['solar_battery_status'] ?? '') == 'good')
                                                echo 'selected'; ?>>
                                                अच्छी (Good)</option>
                                            <option value="poor" <?php if (($edit_data['solar_battery_status'] ?? '') == 'poor')
                                                echo 'selected'; ?>>
                                                खराब (Poor)</option>
                                        </select>
                                    </div>
                        ...
                        <div style="margin-bottom: 20px; margin-left: 10px; margin-right: 10px;">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="internet_conn" class="form-control"
                                            ...
                                            <option value="">-- चुनें --</option>
                                            <option value="yes" <?php if (($edit_data['internet_conn'] ?? '') == 'yes')
                                                echo 'selected'; ?>>
                                                हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['internet_conn'] ?? '') == 'no')
                                                echo 'selected'; ?>>
                                                नहीं
                                            </option>
                                        </select>
                                ...
                                <div class="col-md-4 internet_yes_row"
                                    style="<?php echo (($edit_data['internet_conn'] ?? '') == 'yes') ? 'display:block;' : 'display:none;'; ?>">
                                    <div class="form-group">
                                        <label>सर्विस प्रोवाइडर का नाम</label>
                                        <select name="internet_provider" class="form-control">
                                            <option value="">-- चुनें --</option>
                                            <option value="bsnl" <?php if (($edit_data['internet_provider'] ?? '') == 'bsnl')
                                                echo 'selected'; ?>>
                                                BSNL</option>
                                            <option value="jio" <?php if (($edit_data['internet_provider'] ?? '') == 'jio')
                                                echo 'selected'; ?>>JIO
                                            </option>
                                            <option value="vodafone" <?php if (($edit_data['internet_provider'] ?? '') == 'vodafone')
                                                echo 'selected'; ?>>Vodafone</option>
                                            <option value="airtel" <?php if (($edit_data['internet_provider'] ?? '') == 'airtel')
                                                echo 'selected'; ?>>Airtel</option>
                                            <option value="sdwan" <?php if (($edit_data['internet_provider'] ?? '') == 'sdwan')
                                                echo 'selected'; ?>>
                                                SDWAN</option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-4 internet_yes_row"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="internet_bill_paid" class="form-control">
                                            <option value="yes" <?php if (($edit_data['internet_bill_paid'] ?? '') == 'yes')
                                                echo 'selected'; ?>>हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['internet_bill_paid'] ?? '') == 'no')
                                                echo 'selected'; ?>>नहीं
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-4 internet_yes_row"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="internet_active" class="form-control">
                                            <option value="yes" <?php if (($edit_data['internet_active'] ?? '') == 'yes')
                                                echo 'selected'; ?>>हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['internet_active'] ?? '') == 'no')
                                                echo 'selected'; ?>>
                                                नहीं
                                            </option>
                                        </select>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="water_availability" class="form-control">
                                            <option value="">-- चुनें --</option>
                                            <option value="yes" <?php if (($edit_data['water_availability'] ?? '') == 'yes')
                                                echo 'selected'; ?>>हाँ
                                            </option>
                                            <option value="no" <?php if (($edit_data['water_availability'] ?? '') == 'no')
                                                echo 'selected'; ?>>नहीं
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="water_status" class="form-control">
                                            <option value="operational" <?php if (($edit_data['water_status'] ?? '') == 'operational')
                                                echo 'selected'; ?>>हाँ (संचालित)</option>
                                            <option value="non_operational" <?php if (($edit_data['water_status'] ?? '') == 'non_operational')
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="has_loan" class="form-control" onchange="toggleLoan(this.value)">
                                        <option value="">-- चुनें --</option>
                                        <option value="yes" <?php if (($edit_data['has_loan'] ?? '') == 'yes')
                                            echo 'selected'; ?>>हाँ
                                        </option>
                                        <option value="no" <?php if (($edit_data['has_loan'] ?? '') == 'no')
                                            echo 'selected'; ?>>नहीं
                                        </option>
                                    </select>
                                </div>
                        ...
                        <div class="row loan_sub"
                            style="<?php echo (($edit_data['has_loan'] ?? '') == 'yes') ? 'display:flex;' : 'display:none;'; ?>">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>एन.सी.डी.सी. (लाख रुपये मे)</label>
                                    <input type="number" step="0.01" name="ncdc_loan" class="form-control"
                                        value="<?php echo ($edit_data['ncdc_loan'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="bank_loan" class="form-control"
                                        value="<?php echo ($edit_data['bank_loan'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="upcb_loan" class="form-control"
                                        value="<?php echo ($edit_data['upcb_loan'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="dcb_loan" class="form-control"
                                        value="<?php echo ($edit_data['dcb_loan'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="other_loan_desc" class="form-control"
                                        value="<?php echo ($edit_data['other_loan_desc'] ?? ''); ?>" placeholder="विवरण लिखें">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="other_loan_amount" class="form-control"
                                        value="<?php echo ($edit_data['other_loan_amount'] ?? ''); ?>" placeholder="0.00">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered" id="assets_loan_table">
                                        ...
                                        <tbody>
                                            <?php
                                            $assets_data = json_decode(($edit_data['assets_loan_data'] ?? ''), true) ?: [[]];
                        ...
                        <div class="row">
                            <div class="col-sm-3 form-group">
                                ...
                                <select name="elec_bill_status" class="form-control"
                                    ...
                                    <option value="">-- चुनें --</option>
                                    <option value="yes" <?php echo (($edit_data['elec_bill_status'] ?? '') == 'yes') ? 'selected="selected"' : ''; ?>>हाँ</option>
                                    <option value="no" <?php echo (($edit_data['elec_bill_status'] ?? '') == 'no') ? 'selected="selected"' : ''; ?>>
                                        नहीं</option>
                                </select>
                                <div id="elec_bill_amount_div"
                                    style="display: <?php echo (($edit_data['elec_bill_status'] ?? '') == 'yes') ? 'block' : 'none'; ?>; margin-top: 10px;">
                                    <input type="number" step="0.01" name="elec_bill" class="form-control"
                                        placeholder="धनराशि भरें" value="<?php echo ($edit_data['elec_bill'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="col-sm-3 form-group">
                                <label>(II) कर</label>
                                <select name="other_dues_tax_status" class="form-control"
                                    ...
                                    <option value="">-- चुनें --</option>
                                    <option value="yes" <?php echo (($edit_data['other_dues_tax_status'] ?? '') == 'yes') ? 'selected="selected"' : ''; ?>>हाँ</option>
                                    <option value="no" <?php echo (($edit_data['other_dues_tax_status'] ?? '') == 'no') ? 'selected="selected"' : ''; ?>>नहीं</option>
                                </select>
                                <div id="other_dues_tax_amount_div"
                                    style="display: <?php echo (($edit_data['other_dues_tax_status'] ?? '') == 'yes') ? 'block' : 'none'; ?>; margin-top: 10px;">
                                    <input type="number" step="0.01" name="other_dues_tax" class="form-control"
                                        placeholder="धनराशि भरें" value="<?php echo ($edit_data['other_dues_tax'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="col-sm-3 form-group">
                                ...
                                <select name="other_dues_salary_status" class="form-control"
                                    ...
                                    <option value="">-- चुनें --</option>
                                    <option value="yes" <?php echo (($edit_data['other_dues_salary_status'] ?? '') == 'yes') ? 'selected="selected"' : ''; ?>>हाँ</option>
                                    <option value="no" <?php echo (($edit_data['other_dues_salary_status'] ?? '') == 'no') ? 'selected="selected"' : ''; ?>>नहीं</option>
                                </select>
                                <div id="other_dues_salary_amount_div"
                                    style="display: <?php echo (($edit_data['other_dues_salary_status'] ?? '') == 'yes') ? 'block' : 'none'; ?>; margin-top: 10px;">
                                    <input type="number" step="0.01" name="other_dues_salary" class="form-control"
                                        placeholder="धनराशि भरें" value="<?php echo ($edit_data['other_dues_salary'] ?? ''); ?>">
                                </div>
                            </div>
                            <div class="col-sm-3 form-group">
                                <label>(IV) अन्य</label>
                                <select name="other_dues_other_status" class="form-control"
                                    ...
                                    <option value="">-- चुनें --</option>
                                    <option value="yes" <?php echo (($edit_data['other_dues_other_status'] ?? '') == 'yes') ? 'selected="selected"' : ''; ?>>हाँ</option>
                                    <option value="no" <?php echo (($edit_data['other_dues_other_status'] ?? '') == 'no') ? 'selected="selected"' : ''; ?>>नहीं</option>
                                </select>
                                <div id="other_dues_amount_div"
                                    style="display: <?php echo (($edit_data['other_dues_other_status'] ?? '') == 'yes') ? 'block' : 'none'; ?>; margin-top: 10px;">
                                    <input type="number" step="0.01" name="other_dues_other" class="form-control"
                                        placeholder="धनराशि भरें" value="<?php echo ($edit_data['other_dues_other'] ?? ''); ?>">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table class="table table-bordered" id="employee_table" style="min-width: 1200px;">
                                        ...
                                        <tbody>
                                            <?php
                                            $emps = json_decode(($edit_data['employees_data'] ?? ''), true) ?: [[]];
                                            foreach ($emps as $e): ?>
                                                <tr>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="action_plan" class="form-control"
                                        rows="2"><?php echo ($edit_data['action_plan'] ?? ''); ?></textarea>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function addAssetsLoanRow() {
        var row = `<tr>
            <td><input type="date" name="assets_loan[loan_date][]" class="form-control"></td>
    ...
    $(document).ready(function () {
        ...
        */
        ...
        <?php if (!$is_edit && isset($_SESSION['user_type']) && $_SESSION['user_type'] == 'ar'): ?>
            <?php 
                $pre_division_id = (isset($_SESSION['division_id']) && is_array($_SESSION['division_id']) && count($_SESSION['division_id']) > 0) ? $_SESSION['division_id'][0] : '';
                $pre_district_id = (isset($_SESSION['district_id']) && is_array($_SESSION['district_id']) && count($_SESSION['district_id']) > 0) ? $_SESSION['district_id'][0] : '';
            ?>
            <?php if ($pre_division_id): ?>
                $('#division_name').val('<?php echo $pre_division_id; ?>');
                fill_district('<?php echo $pre_division_id; ?>');
                
                <?php if ($pre_district_id): ?>
                    setTimeout(function () {
                        $('#district_name').val('<?php echo $pre_district_id; ?>');
                        fill_tehseel('<?php echo $pre_district_id; ?>');
                    }, 500);
                <?php endif; ?>
            <?php endif; ?>
        <?php endif; ?>
...
<script>
    ...
    $(document).ready(function() {
        $('#coldStorageForm').on('submit', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                url: 'scripts/ajax_cold_storage.php',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        // Show success card, hide form and history cards
                        $('#historyCard').hide();
                        $('#formCard').hide();
                        // Update preview button URL with the saved record ID
                        if (response.record_id) {
                            $('#preview_btn').attr('onclick', "window.open('cold_storage.php?edit_id=" + response.record_id + "', '_blank');");
                        }
                        $('#successCard').show();
                        $('html, body').animate({ scrollTop: $('#successCard').offset().top - 80 }, 600);
    ...
    // Section V – Storage Capacity: add new row
    function sec_2_nirmit_godown_add_rows() {
        var id = parseInt($("#sec_2_nirmit_godown_id").val()) + 1;
        $("#sec_2_nirmit_godown_id").val(id);
        // remove the button+hidden-field div from the previous last row
        $("#sec_2_nirmit_godown_rows").remove();

        var yearOpts = '<option value="">--Select--</option><option value="1999">2000 से पूर्व</option>';
        for (var y = 2000; y <= 2024; y++) { yearOpts += '<option value="'+y+'">'+y+'</option>'; }
        ...
        var wdraOpts = '<option value="">--select--</option>' +
            '<option value="yes">हाँ</option><option value="no">नहीं</option>';

        var txt = '<div class="row">' +
            '<div class="col-sm-3 form-group"><label>क्षमता (मेट्रिक टन में)</label>' +
            '<input type="text" name="sec_3_b_storage_capacity_'+id+'" id="sec_3_b_storage_capacity_'+id+'" class="form-control chk_number"></div>' +
            '<div class="col-sm-3 form-group"><label>निर्माण का वर्ष</label>' +
            '<select name="sec_3_b_godown_year_'+id+'" id="sec_3_b_godown_year_'+id+'" class="form-control">'+yearOpts+'</select></div>' +
            '<div class="col-sm-3 form-group"><label>WDRA से प्रमाणित है?</label>' +
            '<select name="sec_3_b_wdra_certified_'+id+'" id="sec_3_b_wdra_certified_'+id+'" class="form-control">'+wdraOpts+'</select></div>' +
            '<div class="col-sm-3 form-group"><label>किस योजना के तहत बना है</label>' +
            '<input type="text" name="sec_3_b_godown_type_of_fund_'+id+'" id="sec_3_b_godown_type_of_fund_'+id+'" class="form-control"></div>' +
            '<div class="col-sm-3 form-group"><label>गोदाम की भौतिक स्थिति</label>' +
            '<select name="sec_3_b_godown_status_'+id+'" id="sec_3_b_godown_status_'+id+'" class="form-control">'+statusOpts+'</select></div>' +
            '<div class="col-sm-3 form-group"><label>टिप्पणी</label>' +
            '<input type="text" name="sec_3_b_godown_comment_'+id+'" id="sec_3_b_godown_comment_'+id+'" class="form-control"></div>' +
            '<div class="col-sm-2 form-group my-auto" id="sec_2_nirmit_godown_rows">' +
            '<button type="button" class="btn btn-info" onClick="sec_2_nirmit_godown_add_rows()">नई पंक्ति जोड़े [+]</button>' +
            '<input type="hidden" name="sec_2_nirmit_godown_id" id="sec_2_nirmit_godown_id" value="'+id+'"></div>' +
            '</div>';

        $("#sec_2_nirmit_godown").append(txt);
    }

    // Section VI – Vacant Land: add new row
    function sec_3_c_add_rows() {
        var id = parseInt($("#sec_3_c_id").val()) + 1;
        $("#sec_3_c_id").val(id);
        // remove button div from previous last row
        $("#sec_3_c_rows").remove();

        var roadOpts = '<option value="">--select--</option>' +
        ...
        var txt = '<div class="row">' +
            '<div class="col-sm-3 form-group"><label>क्षेत्रफल (हेक्टेयर में)</label>' +
            '<input type="text" name="sec_3_c_length_'+id+'" id="sec_3_c_length_'+id+'" class="form-control chk_decimal"></div>' +
            '<div class="col-sm-3 form-group"><label>रैक पाइण्ट से दूरी</label>' +
            '<input type="text" name="sec_3_c_rak_distance_'+id+'" id="sec_3_c_rak_distance_'+id+'" class="form-control"></div>' +
            '<div class="col-sm-3 form-group"><label>पहुच मार्ग का प्रकार</label>' +
            '<select name="sec_3_c_paved_road_'+id+'" id="sec_3_c_paved_road_'+id+'" class="form-control">'+roadOpts+'</select></div>' +
            '<div class="col-sm-3 form-group"><label>खाली पड़ी भूमि का किस उपयोग में लाया जा सकता है?</label>' +
            '<input type="text" name="sec_3_c_usage_'+id+'" id="sec_3_c_usage_'+id+'" class="form-control"></div>' +
            '<div class="col-sm-3 form-group"><label>भूमि का फोटो संलग्न करें</label>' +
            '<input type="file" accept=".jpg,.jpeg,.gif,.png,.bmp" class="form-control" name="sec_3_c_food_scheme_image_'+id+'" id="sec_3_c_food_scheme_image_'+id+'"></div>' +
            '<div class="col-sm-2 form-group my-auto" id="sec_3_c_rows">' +
            '<button type="button" class="btn btn-info" onClick="sec_3_c_add_rows();">नई पंक्ति जोड़े</button>' +
            '<input type="hidden" name="sec_3_c_id" id="sec_3_c_id" value="'+id+'"></div>' +
            '</div>';
